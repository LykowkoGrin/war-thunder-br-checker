    ...
    def read_table(self, screenshot) -> list:
        ...
        for i in range(0, 16):
            ...
            name = pytesseract.image_to_string(
                cropped_img, lang="rus+eng", config=self.__scan_config)
            name = name.replace('\n', '')
            name = logo_name + name
    ...
    def __find_bin_logo(bin_img, bin_logo, scale_range=(0.3, 0.4), scale_step=0.01) -> dict:
        ...
        for scale in np.arange(scale_range[0], scale_range[1] + scale_step, scale_step):
            scaled_logo = cv2.resize(
                bin_logo, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)

            if scaled_logo.shape[0] > bin_img.shape[0] or scaled_logo.shape[1] > bin_img.shape[1]:
                continue

            result = cv2.matchTemplate(
                bin_img, scaled_logo, cv2.TM_CCOEFF_NORMED)
